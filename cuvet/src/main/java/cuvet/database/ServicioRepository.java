...
public class ServicioRepository implements IRepository<Servicio, Integer> {
    ...
    public Servicio guardar(Servicio s) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setString(1, s.getTipo().name()); ps.setString(2, s.getDescripcion());
            ps.setBigDecimal(3, s.getPrecio()); ps.setBoolean(4, s.isActivo());
            ps.executeUpdate();
            try (ResultSet rs = ps.getGeneratedKeys()) { if (rs.next()) s.setId(rs.getInt(1)); }
            return s;
        } catch (SQLException e) { throw new DatabaseException("Error al guardar servicio", e); }
    }
    ...
    public Optional<Servicio> buscarPorId(Integer id) {
        ...
        } catch (SQLException e) { throw new DatabaseException("Error al buscar servicio", e); }
    ...
    public List<Servicio> listarTodos() {
        ...
        try (Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery("SELECT * FROM servicios WHERE activo=true ORDER BY tipo")) {
            while (rs.next()) lista.add(mapear(rs));
        } catch (SQLException e) { throw new DatabaseException("Error al listar servicios", e); }
        return lista;
    }
    ...
    public void actualizar(Servicio s) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, s.getDescripcion()); ps.setBigDecimal(2, s.getPrecio());
            ps.setBoolean(3, s.isActivo()); ps.setInt(4, s.getId());
            ps.executeUpdate();
        } catch (SQLException e) { throw new DatabaseException("Error al actualizar servicio", e); }
    }
    ...
    public void eliminar(Integer id) {
        ...
        } catch (SQLException e) { throw new DatabaseException("Error al desactivar servicio", e); }
    }

    public List<Servicio> listarPorTipo(TipoServicio tipo) {
        List<Servicio> lista = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement("SELECT * FROM servicios WHERE tipo=? AND activo=true")) {
            ps.setString(1, tipo.name());
            try (ResultSet rs = ps.executeQuery()) { while (rs.next()) lista.add(mapear(rs)); }
        } catch (SQLException e) { throw new DatabaseException("Error al listar por tipo", e); }
        return lista;
    }

    private Servicio mapear(ResultSet rs) throws SQLException {
        return new Servicio(rs.getInt("id"), TipoServicio.valueOf(rs.getString("tipo")),
